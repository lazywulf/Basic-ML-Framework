...
class SimpleSurface(Module):
    ...
    def forward(self, params):
        ...
        z += .008 * x - .008 * y
        z += __f2(x, y, x_mean=-1.0, y_mean=-1.0, x_sig=0.5, y_sig=0.5, depth=0.3)  # Deep dent
        z += __f2(x, y, x_mean=1.0, y_mean=1.0, x_sig=0.5, y_sig=0.5, depth=0.1)  # Shallow dent
        return z
...
def init_surface_fig():
    x = np.linspace(-5, 2, 1400)
    y = np.linspace(-2, 5, 1400)
    X, Y = np.meshgrid(x, y)
    Z = np.zeros_like(X)
...
def main():
    ...
    init_p = [1.8, -1.7]
    LR = .05
    model = SimpleSurface(x_init=init_p[0], y_init=init_p[1])

    optimizers = {
        'SGD (LR * 10)': [SGD(deepcopy(model), lr=LR * 10), 'r', 'r'],
        'Momentum (LR * 10)': [MSGD(deepcopy(model), lr=LR * 10), 'b', 'b'],
